...
import { type Reminder, type ReminderKind } from '../api/pocketbase';
...
export type RemindersManagerProps = {
  load: () => Promise<Reminder[]>;
  create: (input: Omit<Reminder, 'id' | 'lastDoneAt'>) => Promise<Reminder | null>;
  remove: (r: Reminder) => Promise<void>;
  /** Called after a successful create (e.g. schedule a local notification). */
  afterCreate?: (r: Reminder) => Promise<void>;
  /** If provided, each item shows a "mark taken/done" button (self view only). */
  onDone?: (r: Reminder) => Promise<void>;
  /** Optional content rendered above the list (e.g. a ward wellbeing header). */
  header?: React.ReactNode;
};

/** Shared reminders UI: list + add form. Used for a user's own reminders and by
 *  a guardian managing a ward's reminders. */
export function RemindersManager({ load, create, remove, afterCreate, onDone, header }: RemindersManagerProps) {
  ...
  const reload = () => load().then(setItems);
  useEffect(() => {
    reload();
      ...
      {items.map((r) => {
        ...
        return (
          <View key={r.id} style={styles.card}>
            <Ionicons name={r.kind === 'medication' ? 'medkit' : 'calendar'} size={30} color={colors.primary} />
            ...
            <Pressable accessibilityRole="button" accessibilityLabel={t('reminders.delete')} onPress={() => confirmRemove(r)} hitSlop={8}>
